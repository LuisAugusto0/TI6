...
for b in "${BATCH_SIZES[@]}"; do
  for w in "${WORKERS[@]}"; do
    echo "\n--- Running: batch_size=$b workers=$w ---"
    ts=$(date +%Y%m%d_%H%M%S)
    logf="$OUT_LOGS/run_bs${b}_w${w}_$ts.log"

    # Execute the command and capture exit status; logs go to per-run file
    echo "Command: python yolov5/detectBatchv3.py --weights $WEIGHTS --source $SOURCE --imgsz $IMG_SZ --batch-size $b --workers $w" | tee -a "$logf"
    python yolov5/detectBatchv3.py --weights "$WEIGHTS" --source "$SOURCE" --imgsz $IMG_SZ --batch-size $b --workers $w >> "$logf" 2>&1 || echo "Run FAILED for batch=$b workers=$w (see $logf)" | tee -a "$logf"
